...
const navItems = [
  ...
  { to: '/ai-doctor', icon: Bot, label: 'AI Doctor', badge: 'AI' },
  { to: '/reports', icon: FileText, label: 'Medical Reports', badge: null },
  { to: '/hospitals', icon: Building2, label: 'Hospitals', badge: null },
  { to: '/medicines', icon: Pill, label: 'Medicines', badge: null },
  { to: '/history', icon: Clock, label: 'History', badge: null },
  { to: '/learn', icon: BookOpen, label: 'Learning Lab', badge: '3D' },
...
export default function Sidebar({ user, onLogout }: Props) {
  const allItems = user?.role === 'admin'
    ? [...navItems, { to: '/admin', icon: Shield, label: 'Admin Panel', badge: null }]
    : navItems
  ...
  return (
    <motion.aside
      ...
      className="w-[280px] glass-strong border-r border-white/40 flex flex-col h-full shrink-0 relative overflow-hidden"
    >
